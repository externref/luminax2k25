...
export async function GET({ url }) {
	...
	const predictions = generateWaterQualityPredictions(data);

	return json({ predictions, historical: data });
}

function generateWaterQualityPredictions(historicalData: any[]) {
	...
	for (const hours of hourlyIntervals) {
		...
		const seasonalFactor = 1 + 0.05 * Math.sin((hours / 24) * Math.PI * 2);
		predictedValue *= seasonalFactor;
		
		const noise = (Math.random() - 0.5) * stdDev * 0.2;
		predictedValue += noise;
		
		predictedValue = Math.max(30, Math.min(100, Math.round(predictedValue)));
		
		const uncertainty = stdDev * Math.sqrt(hours / 24);
		const lower = Math.max(30, Math.round(predictedValue - uncertainty * 1.96));
		const upper = Math.min(100, Math.round(predictedValue + uncertainty * 1.96));
